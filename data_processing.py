...
LOCAL_PATH = "csv/"
CSV_PATH = 'dataset/Airline_customer_satisfaction.csv'
...
def global_satisfaction(df):
    satisfaction_dist = df.groupBy("satisfaction").size().reset_index(name='count')
...
def global_client_type_distrib(df):
    customer_type_dist = df.groupBy("Customer Type").size().reset_index(name='count')
...
def age_filtered_distrib(df_age_filtered):
    age_dist_filtered = df_age_filtered[['Age']]
...
def filtered_satisfaction(df_age_filtered):
    filtered_satisfaction_dist = df_age_filtered.groupBy("satisfaction").size().reset_index(name='count')
...
def filtered_client_type_distrib(df_age_filtered):
    filtered_customer_type_dist = df_age_filtered.groupBy("Customer Type").size().reset_index(name='count')
...
def travel_type_distrib(df, loyal):
    if loyal is True:
        df_filtered = df[df["Customer Type"] == "Loyal Customer"]
    else:
        df_filtered = df[df["Customer Type"] == "disloyal Customer"]

    travel_type_dist = df_filtered.groupBy("Type of Travel").size().reset_index(name='count')
...
def travel_type_satisfaction(df, loyal):
    travel_satisfaction_dist = df.groupBy("Type of Travel", "satisfaction").size().reset_index(name='count')
...
def business_satisfaction_per_class(df, loyal):
    ...
    class_satisfaction_distribution = filtered_df.groupBy("Class", "satisfaction").size().reset_index(name='count')
    if loyal is True:
...
def given_notes_per_services(df, loyal, classe):
    ...
    satisfied_means = df_satisfied[parameters].mean().to_frame().transpose()
    ...
    dissatisfied_means = df_dissatisfied[parameters].mean().to_frame().transpose()
...
def flight_distrib(df, loyal, classe):
    distance_dist = df[['Flight Distance']].copy()
    if loyal is True:
...
def satisfaction_per_distance(df, loyal):
    ...
    df_pd = df[['Flight Distance', 'satisfaction']].copy()
...
def get_spark_analyses():
    ...
    if not path.exists('csv/global_means_notes.csv'):
        global_mean_notes(df, rating_columns)
    if not path.exists('csv/ecart_type.csv'):
        global_ecart_type(df, rating_columns)
    if not path.exists('csv/variance.csv'):
        global_variance(df, rating_columns)

    # Distribution de la majorité entre 20 ans et 60 ans
    df_age_filtered = df.loc[(df["Age"] >= 20) & (df["Age"] <= 60)]
    ...
    df_loyal = df_age_filtered[df_age_filtered["Customer Type"] == "Loyal Customer"]
    ...
    df_loyal_eco = df_loyal[df_loyal["Class"] == "Eco"]
    ...
    df_disloyal = df_age_filtered[df_age_filtered["Customer Type"] == "disloyal Customer"]
    ...
    df_disloyal_eco = df_disloyal[
        (df_disloyal["Type of Travel"] == "Business travel") &
        (df_disloyal["Class"] == "Eco")
    ]

    df_disloyal_busi = df_disloyal[
        (df_disloyal["Type of Travel"] == "Business travel") & (df_disloyal["Class"] == "Business")
    ]
    ...
    df_disloyal_distance = df_disloyal_busi[
        (df['Flight Distance'] >= 1100) & (df['Flight Distance'] <= 3000)
    ]
